...
namespace game
{
	results::ResultCode StartEventLoop(conf::Configuration& config)
	{
		while (true)
		{
			...
			IF_RETURN(game::FillVectorUniqueValuesFromRangeAtRandom(
			...
			while (correctNumbers != config.needCorrectNumbers && attempts > 0)
			{
				...
				IF_RETURN(game::compareVectors(decisions.begin(), decisions.end(),
			...
			std::cin >> startAgain;
			ERROR_RETURN_EX(std::cin.fail() || !startAgain, results::eInvalidUserInput);
